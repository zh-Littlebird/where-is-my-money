...
description: 当用户需要根据文字或图片记账，或查询、修改、分析个人财务数据时使用。支持单笔、多笔和拆分交易录入，匹配账户、预算、分类、标签等元数据，并提供交易管理、自动补全、账单、存钱罐、标签、附件、净资产查询、月度报表和收支趋势分析。
---
...
| `scripts/firefly_client.py networth <TOKEN> [YYYY-MM-DD] [CURRENCY_CODE]` | 查询指定日期的净资产（默认今天） |
...
| 净资产查询/月度报告/收支趋势 | `skills/report.md` |
